...
export default function ToursSearching() {
  ...
  const searchState = locationInfo.state || {};
  const { t, locale } = useGlobal();

  const [tourDestination, setTourDestination] = useState(searchState.tourDestination || '');
  const [isTourDestOpen, setIsTourDestOpen] = useState(false);
  const tourDestRef = useRef(null);

  const [tourDate, setTourDate] = useState(searchState.tourDate ? new Date(searchState.tourDate) : null);
  const [isTourDateOpen, setIsTourDateOpen] = useState(false);
  const [calendarCurrentMonth, setCalendarCurrentMonth] = useState(new Date());
  const tourDateRef = useRef(null);

  const [tourOrigin, setTourOrigin] = useState(searchState.tourOrigin || t('all'));
  const [isTourOriginOpen, setIsTourOriginOpen] = useState(false);
  const tourOriginRef = useRef(null);

  const [tourAdults, setTourAdults] = useState(searchState.tourAdults || 1);
  const [tourChildren, setTourChildren] = useState(searchState.tourChildren || 0);
  const [isTourGuestsOpen, setIsTourGuestsOpen] = useState(false);
  const [showGuestLimitMsg, setShowGuestLimitMsg] = useState(false);
  const [childAges, setChildAges] = useState(searchState.childAges || Array(6).fill(1));
  ...
  return (
    <div className="w-full lg:h-[90px] h-auto bg-[#f8f9fa] lg:bg-white shadow-md flex justify-center items-center relative z-40 py-4 lg:py-0">
      <div className="w-full lg:max-w-[1320px] px-4 md:px-6 lg:px-[15px] flex flex-col lg:flex-row items-center gap-3 lg:gap-3 h-full lg:py-4">
        ...
        >
          <svg className="w-6 h-6 text-[#007BFF] shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
        ...
        <div
          ref={tourDateRef}
          className="w-full h-[48px] lg:h-full lg:flex-1 border-[3px] border-[#D9D9D9] rounded-xl bg-white relative flex items-center px-3 gap-2 cursor-pointer shadow-sm hover:shadow-md transition-shadow shrink-0"
          onClick={() => setIsTourDateOpen(!isTourDateOpen)}
        >
          <CalendarIcon className="w-6 h-6 text-[#007BFF] shrink-0" />
          <span className="text-[#180B51] text-base font-medium whitespace-nowrap truncate">
        ...
        <div
          ref={tourGuestsRef}
          className="w-full h-[48px] lg:h-full lg:flex-[1.2] border-[3px] border-[#D9D9D9] rounded-xl bg-white relative flex items-center px-3 gap-2 cursor-pointer shadow-sm hover:shadow-md transition-shadow shrink-0"
          onClick={() => setIsTourGuestsOpen(!isTourGuestsOpen)}
        >
          <GuestIcon className="w-6 h-6 text-[#007BFF] shrink-0" />
          <span className="text-[#180B51] text-base font-medium truncate whitespace-nowrap">
        ...
        <div
          className="w-full h-[48px] lg:w-[60px] lg:h-full bg-[#CC8118] rounded-xl flex items-center justify-center cursor-pointer shrink-0 transition-colors hover:bg-[#b57215] shadow-md"
          onClick={handleSearch}
        >
          <svg className="w-6 h-6 lg:w-7 lg:h-7 text-white font-bold" fill="none" viewBox="0 0 24 24" strokeWidth="3" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
          </svg>
          <span className="lg:hidden ml-2 font-bold text-white text-base">Search</span>
        </div>
