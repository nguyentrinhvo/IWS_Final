...
import { toursForYouData } from '../../data/mockData';
...
const ToursForYou = () => {
  ...
  const updateScrollState = useCallback((rowIndex) => {
    ...
    setScrollStates(prev => {
      ...
      if (!newStates[rowIndex]) newStates[rowIndex] = { canScrollLeft: false, canScrollRight: false };
  ...
  const scrollToCard = (rowIndex, direction) => {
    ...
    if (direction === 'right') {
      const nextCard = cards.find(card => card.offsetLeft > currentScroll + 1);
      if (nextCard) {
        targetScroll = nextCard.offsetLeft;
      }
    } else if (direction === 'left') {
      const previousCards = cards.filter(card => card.offsetLeft < currentScroll - 1);
  ...
  useEffect(() => {
    setScrollStates(toursForYouData.map(() => ({ canScrollLeft: false, canScrollRight: false })));

    const attachListeners = () => {
      toursForYouData.forEach((_, idx) => {
    ...
    const handleResize = () => {
      toursForYouData.forEach((_, idx) => updateScrollState(idx));
    };
  ...
  return (
    <div className="w-full max-w-[1320px] mx-auto py-8 md:py-12 px-4 md:px-6 overflow-visible">
      ...
      <div className="flex flex-col gap-8 md:gap-10 overflow-visible">
        {toursForYouData.map((row, rowIndex) => (
          <div key={row.id} className="flex flex-col md:flex-row w-full gap-6 overflow-visible">
            {/* Left side - Location card (no lift effect) */}
            <div className="w-full md:w-1/4 h-[280px] md:h-[450px] shrink-0 relative rounded-xl overflow-hidden group cursor-pointer z-0">
              <img 
                src={row.location.image} 
                alt={t(row.location.titleKey)} 
            ...
            {/* Right side - Tour slider with per‑card scrolling */}
