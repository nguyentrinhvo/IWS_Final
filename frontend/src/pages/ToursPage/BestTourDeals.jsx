...
const BestTourDeals = () => {
  ...
  useEffect(() => {
    const fetchBestDeals = async () => {
      try {
        setLoading(true);
        const res = await getTours({ size: 6, sort: 'avgRating,desc' });
    ...
    <div className="w-full bg-[#ebf0f2]">
      <div className="w-full max-w-[1320px] mx-auto py-8 md:py-12 px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tours.map((tour) => (
            <Link key={tour.id} to={`/tour/${tour.id}`}>
              <div className="h-[520px] border border-gray-200 rounded-t-xl rounded-b-xl flex flex-col bg-white shadow-md transition-transform duration-300 hover:-translate-y-2 cursor-pointer text-left">
                <img
                ...
                <div className="p-5 flex flex-col flex-1 justify-between">
                  <div>
                    <div className="flex items-center gap-1 mb-2 text-sm flex-wrap">
                      <span className="font-bold">{tour.avgRating || 4.5}</span>
                      <span>/ 5</span>
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-3 text-sm mb-6">
                      <div className="flex items-center gap-1 text-black font-medium">
                        <ClockIcon />
                        <span>{tour.durationDays} {t('days')}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-1 flex-wrap">
                        ...
                        <span className="text-black font-bold whitespace-normal break-words">{tour.departureCity}</span>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-gray-100 pt-4">
                      ...
                      <div className="text-sm text-gray-500 mt-1 break-words">
                        {t('departureDate')}: {tour.departures?.[0] ? new Date(tour.departures[0].departureDate).toLocaleDateString() : 'Contact for dates'}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="flex justify-center mt-8 md:mt-10">
          <Button onClick={handleSeeMore} variant="primary" className="gap-2">
            {t('seeMore')}
            <RightArrowIcon />
          </Button>
        </div>
