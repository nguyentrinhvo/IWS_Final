...
const HotelCard = ({ hotel }) => {
  // Map backend HotelDocument fields to card display
  const name = hotel.name || 'Unknown Hotel';
  const location = `${hotel.address || ''}${hotel.address && hotel.city ? ', ' : ''}${hotel.city || ''}`;
  const stars = hotel.starRating || 0;
  const rating = hotel.avgRating || 0;
  
  // Calculate lowest price from room types
  ...
  const image = hotel.thumbnailUrl || (hotel.images && hotel.images.length > 0 ? hotel.images[0].url : 'https://placehold.co/800x400?text=No+Image');
      ...
      <div className="flex-1 p-5 flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-start mb-1">
            <h3 className="text-xl font-bold text-[#1a2b49] group-hover:text-[#7978E9] transition-colors">{name}</h3>
            <div className="flex items-center bg-blue-50 px-2 py-1 rounded-lg">
               <span className="text-blue-600 font-bold text-sm mr-1">{rating.toFixed(1)}</span>
               <div className="flex gap-0.5">
                  {[...Array(5)].map((_, i) => (
                    <svg key={i} className={`w-3 h-3 ${i < Math.floor(stars) ? 'text-yellow-400' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="flex items-end justify-between pt-4 border-t border-gray-50">
          <div>
            ...
            <p className="text-xl font-black text-orange-500">{minPrice.toLocaleString()} VND</p>
          </div>
