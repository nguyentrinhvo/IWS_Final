    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 transition-all duration-200 hover:shadow-md p-5 flex flex-col lg:flex-row lg:items-center gap-6">
      ...
      <div className="flex-shrink-0 flex items-center gap-4 lg:w-48">
        <div className="w-12 h-12 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center text-2xl">
          {loai_phuong_tien.includes('Train') ? '🚆' : '🚌'}
        </div>
        <div>
          <p className="text-base font-bold text-gray-800 leading-tight">{ten_nha_xe}</p>
          <p className="text-xs text-gray-500 mt-1">{loai_phuong_tien}</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center gap-4">
        ...
        <div className="text-center min-w-[70px]">
          <p className="text-xl font-bold text-[#1a2b49]">{gio_di}</p>
          <p className="text-xs font-semibold text-gray-500 mt-1">{diem_di}</p>
        </div>

        {/* Duration Line */}
        <div className="flex-1 flex flex-col items-center px-2">
          <p className="text-xs text-gray-500 font-medium mb-1">{calculateDuration(gio_di, gio_den)}</p>
        ...
        {/* Arrival */}
        <div className="text-center min-w-[70px]">
          <p className="text-xl font-bold text-[#1a2b49]">{gio_den}</p>
          <p className="text-xs font-semibold text-gray-500 mt-1">{diem_den}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center min-w-[100px]">
        <p className="text-sm font-bold text-gray-700">{so_ghe_con} / {tong_so_ghe}</p>
