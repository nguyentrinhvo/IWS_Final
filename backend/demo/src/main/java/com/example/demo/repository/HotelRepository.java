...
public interface HotelRepository extends MongoRepository<HotelDocument, String> {
    Page<HotelDocument> findByCityIgnoreCaseAndIsActiveTrue(String city, Pageable pageable);

    Page<HotelDocument> findByCityAndStarRatingAndIsActiveTrue(
        String city, Integer starRating, Pageable pageable);

    Page<HotelDocument> findByCityIgnoreCaseAndStarRatingGreaterThanEqualAndIsActiveTrue(
        String city, Integer starRating, Pageable pageable);

    Page<HotelDocument> findByCountryAndIsActiveTrue(String country, Pageable pageable);

    @Query("{ 'city': { '$regex': ?0, '$options': 'i' }, 'isActive': true, " +
           "'roomTypes': { '$elemMatch': { " +
    ...
    @Query("{ '$or': [ { 'city': { '$regex': ?0, '$options': 'i' } }, { 'address': { '$regex': ?0, '$options': 'i' } }, { 'name': { '$regex': ?0, '$options': 'i' } } ], 'isActive': true }")
    Page<HotelDocument> searchByKeyword(String keyword, Pageable pageable);
